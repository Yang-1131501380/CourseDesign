- # F407_TEST 重构计划（详细实施版）
  ...
  ## xrobot 无阻塞优化状态（2026-05-09）

  第二轮优化进一步收敛运行期阻塞点：

  - ST7735 的 SPI 完成/错误回调只关片选和更新状态，不再在回调内启动后续文本。
  - 后续 DMA 文本发送统一由 `ST7735_Process()` 在 `mon` 线程中推进。
  - LCD 忙或队列满时，本轮状态刷新直接放弃，不阻塞控制链路。
  - `mon` 线程改为 `10 ms` 周期非阻塞取事件，避免 `rt_mq_recv()` 长等待拖慢 LCD DMA 推进。
  - 事件队列满时保留“丢旧事件”策略，并增加低频丢弃计数日志。
